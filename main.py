...
from core.matchmaking import add_to_queue, force_start_queue  
...
async def command_join(message):
    ...
    if result["status"] == "already_in_queue":
        await bot.reply_to(message, f"⚠️ Ya estás en la lista de espera de este grupo, @{username}. ({result['current_count']}/5)")
        return
    ...
    if result["status"] == "queued":
        await bot.send_message(
            ...
            f"<i>💡 Si no quieren esperar a los 5, envíen <b>/go</b> para iniciar (Mínimo 2 jugadores).</i>",
            reply_markup=markup
        )
...
async def command_go(message):
    ...
    if result["status"] == "room_created":
        ...
        await bot.send_message(
            chat_group_id,
            f"⚡ <b>¡Partida iniciada con /go!</b>\nCreando sala con los {room['players_count']} jugadores en espera de este grupo.\n\n"
...
async def main():
    ...
    # 🌍 TRUCO OBLIGATORIO PARA RENDER: Puerto dinámico HTTP
    import os
    ...
    class FakeWebhookServer(BaseHTTPRequestHandler):
        def do_GET(self):
            ...
            self.end_headers()
            self.wfile.write(b"Servidor del Bot operando activamente.")
    ...
    try:
        ...
        logger.info(f"🌍 Puerto web simulado activo en el puerto {port}")
    except Exception as e:
        logger.error(f"Error HTTP simulado: {e}")
